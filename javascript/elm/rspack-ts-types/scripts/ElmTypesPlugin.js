...
/**
 * Removes `<resource>.d.ts` for any `.elm` file that was deleted since the
 * last build, so a removed entry module doesn't leave a stale type
 * definition behind.
 *
 ...
 */
function removeStaleTypeDefinitions(compiler, test, logger) {
...
/**
 * Generates `.d.ts` files for every successfully compiled `.elm` module.
 *
 * @param {import("webpack").Compiler} compiler
 * @param {import("webpack").Compilation} compilation
 * @param {RegExp} test
 * @param {import("webpack").Logger} logger
 * @param {Promise<Function>} generateTypeScriptPromise
 */
function generateTypeDefinitionsOnSucceed(
  compiler,
  compilation,
  test,
  logger,
  generateTypeScriptPromise,
) {
  ...
  compilation.hooks.succeedModule.tap(PLUGIN_NAME, (module) => {
    ...
    if (!resource || !test.test(resource)) {
      return;
    }
 ...
 */
function generate(resource, compiledJs, logger, context, generateTypeScriptPromise) {
  return generateTypeScriptPromise.then((generateTypeScript) => {
 ...
 * @example
 * elmTypesPlugin()
 * elmTypesPlugin({ test: /\.elm$/ })
 */
module.exports = ({ test = /\.elm$/ } = {}) => {
  const generateTypeScriptPromise = import("elm-ts-types").then(
  ...
  return {
    apply(compiler) {
      compiler.hooks.compilation.tap(PLUGIN_NAME, (compilation) => {
        ...
        removeStaleTypeDefinitions(compiler, test, logger);
        generateTypeDefinitionsOnSucceed(
          compiler,
          compilation,
          test,
          logger,
