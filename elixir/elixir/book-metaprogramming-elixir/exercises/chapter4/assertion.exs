...
defmodule Assertion.Asserts do
  ...
  def assert_raise(exception, exception), do: :ok
  def assert_raise(_exception, nil) do
    { :fail, """
      Expected an exception to be raised.
      """ }
  end
  def assert_raise(expected_exception, actual_exception) do
    {:fail, """
      Expected to be raised: #{expected_exception}
      Instead got:           #{actual_exception}
      """ }
  end
end
